    ...
    def move_all_poly(self, dim, amount, sign):
        ...
        for polygon in self.polygons:
            ...
            polygon.source.A.point.set_coords(
                polygon.source.A.point.X + valueX,
                polygon.source.A.point.Y + valueY,
                polygon.source.A.point.Z + valueZ,
            )
            polygon.source.B.point.set_coords(
                polygon.source.B.point.X + valueX,
                polygon.source.B.point.Y + valueY,
                polygon.source.B.point.Z + valueZ,
            )
            polygon.source.C.point.set_coords(
                polygon.source.C.point.X + valueX,
                polygon.source.C.point.Y + valueY,
                polygon.source.C.point.Z + valueZ,
            )

            if hasattr(polygon.source, "D"):
                polygon.source.D.point.set_coords(
                    polygon.source.D.point.X + valueX,
                    polygon.source.D.point.Y + valueY,
                    polygon.source.D.point.Z + valueZ,
                )
    ...
    def copy_polygon_to_XOffset(self, copyingPolygon, xOffset, texture):
        ...
        if sides == 4:
            ...
            polygon.source.D.point.set_coords(
                ...
                copyingPolygon.source.D.point.Z,
            )
        if texture:
