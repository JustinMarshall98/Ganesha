...
class Resource(object):
    ...
    def write(self):
        ...
        if new_sectors > old_sectors:
            print(
                "WARNING: File has grown from %u sectors to %u sectors!"
                % (old_sectors, new_sectors)
            )
        elif new_sectors < old_sectors:
            print(
                "Note: File has shrunk from %u sectors to %u sectors."
                % (old_sectors, new_sectors)
            )
...
class Resources(object):
    ...
    def put_polygons(self, polygons, toc_offset=0x40):
        ...
        polygons_data = pack(
            "<4H", *[len(x) for x in [tex_tri, tex_quad, untex_tri, untex_quad]]
        )
